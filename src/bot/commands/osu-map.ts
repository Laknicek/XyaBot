import { SlashCommandBuilder, EmbedBuilder, MessageFlags, ChatInputCommandInteraction, Attachment } from 'discord.js';
import { Command } from '../types';
import { getUser, updateUser, getGuildSetting } from '../db';
    ...
    .addStringOption(option =>
        option.setName('mode')
            ...
            .addChoices(
                ...
                { name: 'Taiko (1)', value: '1' },
                { name: 'Catch (2)', value: '2' },
                { name: 'Mania (3)', value: '3' }
            ))
    .addBooleanOption(option =>
        option.setName('use_lora')
...
export const execute: Command['execute'] = async (interaction) => {
    ...
    if (settings.shop_enabled === 0 || settings.shop_osu_enabled === 0) {
        return interaction.reply({ content: '🚫 The **Osu Mapping Shop** is currently closed.', flags: [MessageFlags.Ephemeral] });
    }
    ...
    if (!allowedTypes.includes(audioFile.contentType || '')) {
        return interaction.reply({ content: '❌ Invalid file type. Please upload an MP3, OGG, or WAV file.', flags: [MessageFlags.Ephemeral] });
    }

    if (audioFile.size > 20 * 1024 * 1024) { // 20MB limit
        return interaction.reply({ content: '❌ File too large. Max 20MB.', flags: [MessageFlags.Ephemeral] });
    }

    // --- DEDUCT GEMS & ENQUEUE ---
    updateUser(interaction.user.id, { currency: user.currency - totalCost });
...
const processQueue = async () => {
    ...
    const user = getUser(userId, job.username); // Refresh user data just in case
    ...
    try {
        ...
        const updateEmbed = async (phase: string, pct: number) => {
            const now = Date.now();
            if (now - lastUpdate < 3000 && pct < 100) return;
            lastUpdate = now;
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0xFF69B4)
                .addFields(
                    { name: 'Target', value: `${difficulty}⭐ ${['Std', 'Taiko', 'CtB', 'Mania'][parseInt(mode)]}`, inline: true },
                    { name: 'BPM', value: `${bpm}`, inline: true },
                    { name: 'Queue', value: `${queue.length} waiting`, inline: true },
                    { name: 'Map Stats', value: `CS${stats.cs} AR${stats.ar} OD${stats.od} HP${stats.hp}`, inline: false },
                    { name: 'System', value: `🖥️ CPU: ${cpuCount} Cores | 🧠 RAM: ${ramUsed}/${ramTotal} GB`, inline: false },
        ...
        try {
            ...
            const bpmProcess = spawn(getPythonPath(), ['detect_bpm.py', audioPath], { cwd: MAPPERATORINATOR_PATH });

            bpm = await new Promise<number>((resolve) => {
                let data = '';
        ...
        const getRandom = (min: number, max: number) => parseFloat((Math.random() * (max - min) + min).toFixed(1));

        const calculateStats = (stars: number) => {
        ...
        await new Promise<void>((resolve, reject) => {
            const child = spawn(getPythonPath(), args, { cwd: MAPPERATORINATOR_PATH, shell: false });
            let errorOutput = '';
            let fullLog = '';
            ...
            child.stderr.on('data', async (data) => {
                ...
                if (str.includes('Generating timing')) currentPhase = 'Generating Timing...';
                if (str.includes('Generating events')) currentPhase = 'Generating Hit Objects...';
                if (str.includes('Refining')) currentPhase = 'Refining Positions...';
            });
            ...
            child.on('close', async (code) => {
                ...
                if (code !== 0) {
                    ...
                    updateUser(userId, { currency: user.currency }); // Refund manually here as we handled the reply
                    // Refresh user balance for adding back cost
                    const currentUser = getUser(userId, job.username);
                    updateUser(userId, { currency: currentUser.currency + cost });
                ...
                // Find .osz file
                // Recurse to find .osz
                const findOsz = (dir: string): string | null => {
                ...
                if (oszPath) {
                    try {
                        const resultEmbed = new EmbedBuilder()
                            ...
                            .setColor(0x00FF00)
                            .addFields(
                                { name: 'Stats', value: `CS${stats.cs} AR${stats.ar} OD${stats.od} HP${stats.hp}`, inline: true },
                                { name: 'BPM', value: `${bpm}`, inline: true }
                            )
                ...
                // Cleanup
                setTimeout(() => { try { fs.rmSync(tempDir, { recursive: true, force: true }); } catch { } }, 10000);
                resolve();
            });
