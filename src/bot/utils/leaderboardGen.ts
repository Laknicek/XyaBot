...
export async function generateLeaderboardImage(topUsers: any[]) {
    const width = 900;
    const height = 1100;
    ...
    drawSprinkles(ctx, width, height, 120);

    // Decorations
    drawLollipop(ctx, 80, 80, 50);
    drawWrappedCandy(ctx, 820, 80, 70, '#FF1493');
    drawWrappedCandy(ctx, 80, 1020, 60, '#00FFFF');
    drawLollipop(ctx, 820, 1000, 50);
    ...
    // Main Glass Panel
    const listX = 50; const listY = 140; const listW = 800; const listH = 900;
    drawCandyGlassCard(ctx, listX, listY, listW, listH, 40);

    const rowH = 80; const rowGap = 15;
    topUsers.forEach((user, i) => {
        ...
        // Top 3 Sparkles
        if (i < 3) {
            drawSparkle(ctx, listX + listW - 30, y + 10, 12);
        }
    });
