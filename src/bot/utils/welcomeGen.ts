...
export async function generateWelcomeImage(username: string, avatarUrl: string | null) {
    const width = 1000;
    const height = 450;
    ...
    // --- Glassy Card ---
    const cardX = 40; const cardY = 40; const cardW = 920; const cardH = 370;
    ctx.save();
    ...
    function drawFlower(fx: number, fy: number, size: number, color: string) {
        ctx.save();
        ctx.translate(fx, fy);
        ctx.fillStyle = color;
        for (let i = 0; i < 5; i++) {
