...
export interface Theme {
    ...
    description: string;
    emoji: string;
    color: ColorResolvable; // Hex color for embeds
    image?: string;         // Optional large image for embeds
    banner?: string;        // Optional top category name (e.g. "Merry Christmas")
    format: {
        open: string[];     // e.g. ["୨", "ʚ"]
        close: string[];    // e.g. ["୧", "ɞ"]
        separator: string[]; // e.g. ["・", " :: "]
        suffix: string[];   // e.g. ["", " 🍃"]
        tree: {
            top: string;    // e.g. "╭" or "┌" - FIRST item
            branch: string; // e.g. "├" or "│" - MIDDLE items
            leaf: string;   // e.g. "╰" or "└" - LAST item
            alone: string;  // e.g. "─" or "╰" or "•"
        }
    };
    categoryFormat?: {
        prefix: string[];
...
const getDynamicDecorationRegex = (): RegExp => {
    const parts: string[] = [];
    ...
    Object.values(THEMES).forEach(theme => {
        ...
        if (f.close) parts.push(...f.close);
        if (f.separator) parts.push(...f.separator);
        if (f.suffix) parts.push(...f.suffix);
        if (f.tree) parts.push(f.tree.top, f.tree.branch, f.tree.leaf, f.tree.alone);

        // Collect category format parts
        if (theme.categoryFormat) {
            const c = theme.categoryFormat;
            if (c.prefix) parts.push(...c.prefix);
            if (c.open) parts.push(...c.open);
            if (c.close) parts.push(...c.close);
...
let DYNAMIC_DECORATION_REGEX: RegExp | null = null;

// Position type for tree alignment
export type ChannelPosition = 'top' | 'branch' | 'leaf' | 'alone' | 'category' | 'none';

export const generateChannelName = (originalName: string, themeId: string, index: number, position: ChannelPosition): string => {
    ...
    if (isCategory) {
        ...
        if (position === 'top') prefix = theme.format.tree.top;
        else if (position === 'branch') prefix = theme.format.tree.branch;
        else if (position === 'leaf') prefix = theme.format.tree.leaf;
        else if (position === 'alone') prefix = theme.format.tree.alone;
    }
...
export const getPreview = (channels: { name: string, id: string, type: number, rawPosition: number, parentId?: string | null }[], themeId: string): { id: string, oldName: string, newName: string }[] => {
    ...
    sorted.forEach((c) => {
        if (c.type === 4) { // Category
            result.push({
                id: c.id,
                oldName: c.name,
                newName: generateChannelName(c.name, themeId, globalIndex, 'category')
            });
        } else {
            // Find siblings
            const key = c.parentId || 'root';
            ...
            let pos: ChannelPosition = 'branch';
            if (isAlone) pos = 'alone';
            else if (isFirst) pos = 'top'; // top takes precedence over branch if !alone
            else if (isLast) pos = 'leaf';

            result.push({
                id: c.id,
                oldName: c.name,
                newName: generateChannelName(c.name, themeId, globalIndex + myIndexInside, pos)
            });
        }
        globalIndex++;
    });
