...
import db, { getMemories, getMilestoneTitle, logBotStat } from './db';
...
// WYR Generation
export const generateWyrScenario = async (): Promise<{ question: string, optionA: string, optionB: string }> => {
    if (!API_KEY) return { question: 'Would You Rather...', optionA: 'Be a bird', optionB: 'Be a fish' };
...
export function getCurrentMood(): { mood: string, emoji: string, effect: string } {
    const hour = new Date().getHours();

    // Base mood from time of day
    let baseMood: { mood: string, emoji: string, effect: string };
    if (hour >= 6 && hour < 11) baseMood = { mood: 'Energetic', emoji: '☀️', effect: 'excited, uses exclamation marks, upbeat' };
    else if (hour >= 11 && hour < 17) baseMood = { mood: 'Chill', emoji: '😌', effect: 'relaxed, lowercase everything, laid-back vibes' };
    else if (hour >= 17 && hour < 21) baseMood = { mood: 'Creative', emoji: '🎵', effect: 'talks about music, more expressive, artistic' };
    else if (hour >= 21 || hour < 0) baseMood = { mood: 'Sleepy', emoji: '😴', effect: 'short responses, occasional yawning, cozy vibes' };
    else baseMood = { mood: 'Chaotic', emoji: '🌙', effect: 'random, unpredictable, meme-y, chaotic energy' };

    // Context shift: check recent sentiment balance
    try {
        const recent = db.prepare(
            'SELECT sentiment FROM interactions ORDER BY timestamp DESC LIMIT 10'
...
function buildSystemPrompt(userId?: string, friendshipPoints?: number): string {
    const mood = getCurrentMood();
    const milestone = friendshipPoints !== undefined ? getMilestoneTitle(friendshipPoints) : '❓ Stranger';

    // Get user memories if available
    let memoriesBlock = '';
    if (userId) {
        const memories = getMemories(userId, 8);
...
const executeTextAI = async (prompt: string, history: { role: "user" | "model", parts: string }[], userId?: string, friendshipPoints?: number, images?: string[]) => {
    if (!API_KEY) throw new Error("Google API Key is missing");
...
export const generateResponse = (prompt: string, history: { role: "user" | "model", parts: string }[], userId?: string, friendshipPoints?: number, images?: string[]) => {
    return new Promise<{ text: string, sentiment: string, lang: string, memories: string[] }>((resolve, reject) => {
        requestQueue.push({ type: 'TEXT', data: { prompt, history, userId, friendshipPoints, images }, resolve, reject });
        processQueue();
    });
};

export const generateVoiceResponse = (transcribedText: string, relationship: string = "New User", history: { role: "user" | "model", parts: string }[] = [], userId?: string, friendshipPoints?: number) => {
...
4. COMMANDS: If user asks for command, output [CMD:name].]`;
    ...
    return generateResponse(prompt, history, userId, friendshipPoints);
};
...
export const scanAttachment = async (url: string, mimeType: string): Promise<{ safe: boolean, reason?: string }> => {
    ...
    try {
        ...
        if (!imageBase64) return { safe: true };
...
// Event Verification
export const verifyImageContent = async (url: string, requirement: string): Promise<boolean> => {
    if (!API_KEY) return true; // Fail safe (pass) if no API key
