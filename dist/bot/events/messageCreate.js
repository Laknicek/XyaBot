...
const isContentValidForXp = (content) => {
...
const execute = async (message) => {
    ...
    if (message.guild) {
        (0, db_1.logMessage)(message.author.id, message.guild.id, message.channel.id);
        (0, db_1.updateUserActivityByHour)(message.author.id, hour);
        (0, db_1.updateUserActivityByDay)(message.author.id, day);
        (0, db_1.updateMessagesPerChannel)(message.author.id, message.channel.id);
        (0, db_1.updateUser)(message.author.id, { last_active_time: now });
    }
    ...
    if (message.guild) {
        ...
        if (spamData.count > SPAM_THRESHOLD) {
            ...
            if (newWarnings === 1) {
                ...
                (0, db_1.updateUser)(message.author.id, { warnings: newWarnings });
            }
            else {
                ...
                try {
                    if (message.member?.moderatable) {
                        ...
                        if (message.channel.isSendable())
                            await message.channel.send(`Xya wants to timeout ${message.author}, but they are too powerful! 🥺`);
                    }
                    (0, db_1.updateUser)(message.author.id, {
    ...
    const toxicSentiment = (0, moderation_1.checkToxicity)(message.content);
    if (toxicSentiment !== "Neutral") {
        console.warn(`[TOXICITY DETECTED] ${message.author.username} - Sentiment: ${toxicSentiment}`);
        let warnKey = "warnings_rude";
        let limit = 5;
        let disgustBase = 20;
        let friendshipLoss = 10;
        if (toxicSentiment === "Rude") {
            warnKey = "warnings_rude";
            limit = 5;
            disgustBase = 20;
            friendshipLoss = 10;
        }
        if (toxicSentiment === "Hateful") {
            warnKey = "warnings_hateful";
            limit = 4;
            disgustBase = 50;
            friendshipLoss = 30;
        }
        if (toxicSentiment === "Racist") {
            warnKey = "warnings_racist";
            limit = 2;
            disgustBase = 150;
            friendshipLoss = 100;
        }
        ...
        if (newWarnings >= limit) {
            ...
            if (Date.now() - (user.last_offense_time || 0) > PUNISH_RESET_TIME)
                currentLevel = 0;
...
You've been timed out for **${timeoutMinutes}m**. Please be kinder! 🥺`);
        ...
        (0, db_1.updateUser)(message.author.id, updateData);
        return; // STOP: No XP for toxic messages
    }
    // --- Wordle Game Logic ---
    const activeWordle = (0, db_1.getWordleGame)(message.author.id);
    if (activeWordle && /^[a-zA-Z]+$/.test(message.content) && !message.content.includes(' ')) {
        ...
        if (guess.length !== targetLength) {
            ...
            const guesses = JSON.parse(activeWordle.guesses);
            if (guesses.includes(guess)) {
                ...
                if (guess === activeWordle.word) {
                    const buffer = (0, wordleGen_1.generateWordleBoard)(activeWordle.word, guesses);
                    const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: 'wordle_win.png' });
                    ...
                    (0, db_1.deleteWordleGame)(message.author.id);
                    const newXp = user.xp + 100;
                    const newCurrency = user.currency + 50;
                    (0, db_1.updateUser)(message.author.id, { currency: newCurrency, xp: newXp });
                    (0, db_1.logEconomyTransaction)(message.author.id, 'wordle_win', 50, 'wordle_game', newCurrency);
                }
                else if (tries >= 6) {
                    const buffer = (0, wordleGen_1.generateWordleBoard)(activeWordle.word, guesses);
                    const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: 'wordle_loss.png' });
                    const embed = new discord_js_1.EmbedBuilder()
                        .setColor(0xFF0000)
                        .setTitle('Oh no...')
                    ...
                    (0, db_1.saveWordleGame)(message.author.id, activeWordle.word, guesses, tries);
                    const buffer = (0, wordleGen_1.generateWordleBoard)(activeWordle.word, guesses);
    ...
    if (isContentValidForXp(message.content)) {
        ...
        (0, db_1.updateUser)(message.author.id, { xp: newXp });
    ...
    if (shouldRespond) {
        ...
        if (guildId) {
            ...
            if (settings?.maintenance_mode || settings?.disable_chat)
                return;
        }
        ...
        try {
            const rawHistory = (0, db_1.getInteractions)(message.author.id, 6);
            const history = rawHistory.reverse().flatMap(h => [
                { role: "user", parts: h.content },
                { role: "model", parts: h.response }
            ]);
            const relationship = (0, db_1.getUserRelationship)(message.author.id);
            ...
            if (hasImages) {
                ...
                const validImages = results.filter((b) => b !== null);
            ...
            let { text, sentiment, lang, memories } = await (0, ai_1.generateResponse)(prompt, history, message.author.id, friendshipPts, imageBase64List.length > 0 ? imageBase64List : undefined);
            ...
            if (cmdMatch) {
                ...
                const cmd = commands_1.commands.get(cmdName);
                if (cmd) {
                    try {
            ...
            if (memories && memories.length > 0) {
                for (const fact of memories) {
                    (0, db_1.saveMemory)(message.author.id, fact);
            ...
            // --- Relationship Update ---
            const userStats = (0, db_1.getUser)(message.author.id, message.author.username);
            ...
            else if (sentiment === "Hateful" || sentiment === "Racist") {
                fDelta = -50;
                dDelta = 50;
            } // Was -100, +100
            else {
            ...
            const thresholds = (0, db_1.getMilestoneThresholds)();
            for (const threshold of thresholds) {
                if (oldFriendship < threshold && newFriendship >= threshold) {
                    const title = (0, db_1.getMilestoneTitle)(newFriendship);
            ...
            (0, db_1.logInteraction)(message.author.id, message.author.username, message.content || '(image)', text, sentiment);
