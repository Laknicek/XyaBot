...
class EventSystem {
    constructor() {
        this.client = null;
        this.activeEvents = new Map(); // guildId -> currentEvent
        this.eventTimeout = null;
        this.winners = new Map(); // eventId -> Set<userId> (for preventing double rewards)
        // --- DEFINED EVENTS ---
        this.events = [
            // PASSIVE EVENTS
            {
    ...
    async triggerRandomEvent() {
        ...
        for (const guild of this.client.guilds.cache.values()) {
            ...
            if (!settings?.events_channel_id)
                continue; // STRICT: Only spawn in configured channel
            const channel = guild.channels.cache.get(settings.events_channel_id);
            ...
            if (eventInstance.onStart)
                await eventInstance.onStart(channel);
            // Schedule End
            setTimeout(() => this.endEvent(guild.id, channel), eventInstance.duration);
        }
        this.scheduleNextEvent();
    }
    async endEvent(guildId, channel) {
    ...
    async handleMessage(message) {
        ...
        if (!event)
            return;
        const settings = (0, db_1.getGuildSetting)(message.guild.id);
        if (message.channelId !== settings?.events_channel_id)
            return; // Only process in event channel
        ...
        if (event.type === 'passive') {
            if (event.rewardCoins) {
                ...
                if (message.content.length > 2) {
                    ...
                    (0, db_1.updateUser)(message.author.id, { currency: user.currency + event.rewardCoins });
                }
            }
        ...
        if (event.type === 'interactive') {
            ...
            if (event.requirement && message.attachments.size > 0) {
                ...
                if (isImage) {
                    ...
                    const isValid = await (0, ai_1.verifyImageContent)(attachment.url, event.requirement);
                    if (isValid) {
            ...
            // Custom Logic
            if (event.onMessage)
                await event.onMessage(message);
        }
    }
    async awardPrize(message, amount, eventName) {
        ...
        const user = (0, db_1.getUser)(message.author.id, message.author.username);
        (0, db_1.updateUser)(message.author.id, { currency: user.currency + amount });
        await message.reply(`🎉 **Correct!** You won **${amount} Gems**! (${eventName})`);
    }
    getActiveEvent(guildId) {
        return this.activeEvents.get(guildId);
    }
}
exports.eventSystem = new EventSystem();
