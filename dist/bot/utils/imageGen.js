...
async function generateProfileImage(user, avatarUrl) {
    const width = 900;
    const height = 600;
    ...
    (0, candyUtils_1.drawSprinkles)(ctx, width, height, 80);
    // Floating Decorations
    (0, candyUtils_1.drawWrappedCandy)(ctx, 60, 60, 60, '#FF69B4');
    (0, candyUtils_1.drawLollipop)(ctx, 840, 60, 45);
    (0, candyUtils_1.drawWrappedCandy)(ctx, 840, 540, 60, '#00CED1');
    (0, candyUtils_1.drawLollipop)(ctx, 60, 540, 40);
    (0, candyUtils_1.drawSparkle)(ctx, 450, 40, 20);
    // Main Card
    const cardX = 40;
    const cardY = 50;
    const cardW = 820;
    const cardH = 500;
    (0, candyUtils_1.drawCandyGlassCard)(ctx, cardX, cardY, cardW, cardH, 40);
    // --- Avatar Section ---
    ...
    ctx.restore();
    // --- Bars Section ---
    function drawStatBar(lx, ly, label, val, max, color, icon) {
        // Label
        ctx.fillStyle = '#555';
    ...
    // --- Badges ---
    const badges = (0, db_1.getBadges)(user.id);
    let bx = 90;
