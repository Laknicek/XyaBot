...
exports.setupVoiceListening = setupVoiceListening;
const voice_1 = require("@discordjs/voice");
const voiceUtils_1 = require("./voiceUtils");
...
const userDebounce = new Set();
const tempDir = path_1.default.join(__dirname, '../../../temp_audio');
if (!fs_1.default.existsSync(tempDir))
    fs_1.default.mkdirSync(tempDir, { recursive: true });
async function setupVoiceListening(connection, client) {
    ...
    receiver.speaking.on('start', (userId) => {
        ...
        if (guildLocks.has(guildId) || userDebounce.has(userId))
            return;
...
async function handleSpeechBurst(receiver, connection, userId, client) {
    ...
    const opusDecoder = new prism_media_1.opus.Decoder({ frameSize: 960, channels: 2, rate: 48000 });
    ...
    const chunks = [];
    const streamCollector = new stream_1.Writable({
    ...
    (0, stream_1.pipeline)(audioStream, opusDecoder, streamCollector, async (err) => {
        audioStream.destroy();
        opusDecoder.destroy();
        ...
        try {
            const userData = (0, db_1.getUser)(userId, "Voice User");
            ...
            const transcribedText = await (0, ai_1.transcribeAudio)(wavBuffer);
            ...
            const relationship = (0, db_1.getUserRelationship)(userId);
            const friendshipPts = userData.friendship_points || 0;
            const rawHistory = (0, db_1.getInteractions)(userId, 3);
            const history = rawHistory.reverse().flatMap(h => [
                { role: "user", parts: h.content },
                { role: "model", parts: h.response }
            ]);
            let { text, lang, sentiment, memories } = await (0, ai_1.generateVoiceResponse)(transcribedText, relationship, history, userId, friendshipPts);
            if (!text)
            ...
            if (activeSpeakers.size > 1) {
                await (0, voiceUtils_1.playTTS)(`Please guys, one by one! I'm trying to listen to ${userData.username}...`, connection, lang);
            ...
            if (cmdMatch) {
                ...
                if (cmd) {
                    try {
                        ...
                        const targetChannel = guild?.channels.cache.find((c) => c.isTextBased());
                        if (targetChannel) {
                            ...
                            const mockInteraction = {
                                ...
                                editReply: async (payload) => {
                                    if (typeof payload === 'string')
                                        return targetChannel.send(`<@${userId}>, ${payload}`);
            ...
            // --- Voice Moderation ---
            const toxicSentiment = (0, moderation_1.checkToxicity)(text);
            if (toxicSentiment !== "Neutral") {
                ...
                await (0, voiceUtils_1.playTTS)(`Please don't be ${toxicSentiment.toLowerCase()}, ${userData.username}... it makes Xya's heart hurt.`, connection);
                (0, db_1.updateUser)(userId, updateData);
                return;
            }
            await (0, voiceUtils_1.playTTS)(text, connection, lang);
            ...
            if (sentiment === "Kind") {
                ...
                fDelta = 3;
                dDelta = -2;
            } // Neutral: slow friendship build + disgust decay
            (0, db_1.updateUser)(userId, {
            ...
            (0, db_1.logInteraction)(userId, userData.username, `[Voice] ${transcribedText}`, text, sentiment);
        }
        catch (e) {
