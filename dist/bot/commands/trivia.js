...
const execute = async (interaction) => {
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(...q.options.map((opt, i) => new discord_js_1.ButtonBuilder()
        .setCustomId(`trivia_${i}`)
        .setLabel(`${OPTION_LABELS[i]} ${opt}`)
        .setStyle(i === 0 ? discord_js_1.ButtonStyle.Primary : i === 1 ? discord_js_1.ButtonStyle.Danger : i === 2 ? discord_js_1.ButtonStyle.Success : discord_js_1.ButtonStyle.Secondary)), new discord_js_1.ButtonBuilder()
    ...
    // Register game
    gameUtils_1.default.register(interaction.user.id, 'trivia', interaction.channelId, () => {
    ...
    collector.on('collect', async (btn) => {
        ...
        if (chosen === q.answer) {
            ...
            const user = (0, db_1.getUser)(btn.user.id, btn.user.username);
            (0, db_1.updateUser)(btn.user.id, {
    ...
    collector.on('end', async (_, reason) => {
        // Remove from manager
        gameUtils_1.default.stopGame(interaction.user.id); // Cleanup map, but don't call stop() again
