...
const COST = 10;
// Payout table
function calculatePayout(s1, s2, s3) {
    if (s1 === '💎' && s2 === '💎' && s3 === '💎')
        return { multiplier: 50, name: 'DIAMOND JACKPOT 💎💎💎' };
    if (s1 === '7️⃣' && s2 === '7️⃣' && s3 === '7️⃣')
        return { multiplier: 25, name: 'TRIPLE SEVENS 7️⃣7️⃣7️⃣' };
    if (s1 === '🌟' && s2 === '🌟' && s3 === '🌟')
        return { multiplier: 15, name: 'STAR COMBO ⭐' };
    if (s1 === '💰' && s2 === '💰' && s3 === '💰')
        return { multiplier: 10, name: 'MONEY BAGS 💰' };
    if (s1 === s2 && s2 === s3)
        return { multiplier: 5, name: 'TRIPLE MATCH!' };
    if (s1 === s2 || s2 === s3 || s1 === s3)
        return { multiplier: 2, name: 'Double Match' };
...
const execute = async (interaction) => {
    ...
    // Deduct cost
    (0, db_1.updateUser)(interaction.user.id, { currency: balance - COST });
    // Spin!
    ...
    if (winnings > 0) {
        (0, db_1.updateUser)(interaction.user.id, { currency: balance - COST + winnings });
    }
    ...
    if (isWin) {
        embed.addFields({ name: `🎉 ${result.name}`, value: `**+${winnings} gems** (${result.multiplier}x)`, inline: true }, { name: '💰 Balance', value: `${balance - COST + winnings} gems`, inline: true });
    }
    else {
        embed.addFields({ name: '💨 No Match', value: `**-${COST} gems**`, inline: true }, { name: '💰 Balance', value: `${balance - COST} gems`, inline: true });
    }
